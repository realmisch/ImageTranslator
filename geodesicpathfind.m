function [xpoints,ypoints] = geodesicpathfind(x1,y1,x2,y2,I)
    ...
    %Out of all paths, find the middle ground and detect any branching
    %points
    ...
    %If bwdistgeodesics fails and generates noncontinuous path, do not plot
    %path
    if max(max(body))~=1
        pnt=nopnt;
    end
    ...
    while pnt<nopnt
        ...
        %If multiple paths are generated, recalculate path based on one of
        %the branches
    ...
    %Path points returned to function call
    xpoints=xlist;
    ypoints=ylist;
end
